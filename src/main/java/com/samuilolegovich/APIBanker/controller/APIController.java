...
import com.samuilolegovich.APIBanker.model.repo.mysql.*;
...
public class APIController {
    @Autowired
    private CustomizedPaymentsJournalRepository customizedPaymentsJournalRepositoryPG;
    @Autowired
    private CustomizedAccountsRepository customizedAccountsRepositoryPG;
    @Autowired
    private PaymentsJournalRepository paymentsJournalRepositoryPG;
    @Autowired
    ...
    public ResponseEntity<AnswerClientID[]> findClient(@PathVariable(value = "id") long id) {
        ...
        clientsRepositoryPG.findById(id).orElseThrow(ClientNotFoundException::new);
        // получаем все счета клиета создаем объекты выводв серелиазуем их и отправляем ответ
        List<Accounts> accountsList = customizedAccountsRepositoryPG.findAllByClientId(id);
...
//    @RequestMapping(value = "/client", method = RequestMethod.POST)
    ...
    public ResponseEntity<AnswerForNewPay> createPayment(@RequestBody NewPay in) {
        ...
        clientsRepositoryPG.findById(in.getSource_acc_id()).orElseThrow(SourceNotFoundException::new);
        clientsRepositoryPG.findById(in.getDest_acc_id()).orElseThrow(DestNotFoundException::new);
    ...
    private AnswerForNewPay receiveInvoicesAndMakeTransfer(NewPay newPay) {
        // получаем счета
        List<Accounts> sourceAccList = customizedAccountsRepositoryPG.findAllByClientId(newPay.getSource_acc_id());
        List<Accounts> destAccList = customizedAccountsRepositoryPG.findAllByClientId(newPay.getDest_acc_id());
    ...
    private AnswerForNewPay makeTransferOfFunds(NewPay newPay, Accounts source, Accounts dest) {
        ...
        paymentsJournalRepositoryPG.save(paymentsJournal);
        AnswerForNewPay answerForNewPay = new AnswerForNewPay(paymentsJournal);
        accountsRepositoryPG.save(source);
        accountsRepositoryPG.save(dest);
    ...
    private AnswerForNewPays[] makeAllPayments(List<NewPay> newPayList) {
        ...
        for (NewPay newPay : newPayList) {
            ...
            if (!clientsRepositoryPG.existsById(newPay.getSource_acc_id())
                    || !clientsRepositoryPG.existsById(newPay.getDest_acc_id())) {
    ...
    private AnswerForPaymentJournal[] getPaymentReport(PaymentJournal paymentJournal) {
        List<PaymentsJournal> paymentsJournalList = customizedPaymentsJournalRepositoryPG
        ...
        Optional<Clients> optionalClientsRecipient = clientsRepositoryPG.findById(paymentJournal.getRecipient_id());
        Optional<Clients> optionalClientsPayer = clientsRepositoryPG.findById(paymentJournal.getPayer_id());
